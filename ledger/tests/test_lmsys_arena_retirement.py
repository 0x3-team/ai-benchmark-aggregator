...
def _source(*, status: str, notes: str | None, parser_config: dict[str, object]) -> dict[str, object]:
    return {
        "id": "lmsys-arena-transition-fixture",
        "benchmark_id": "chatbot_arena",
        "source_name": "LMSYS Arena transition fixture",
...
def test_retiring_lmsys_arena_appends_a_revision_and_preserves_historical_evidence(seeded_db):
    active = _source(status="active", notes=None, parser_config={})
    retired = _source(
        status="inactive",
        notes="Retired LDR-06: third-party fallback is not official result evidence.",
        parser_config={
            "mode": "retired",
            "retirement_reason": "third-party fallback route is not source-reported benchmark evidence",
        },
    )
    with get_session() as session:
        ...
        snapshot = repo.insert_snapshot(
            ...
            raw_content_uri="file:///test-only/lmsys-arena.json",
            content_hash="c" * 64,
        ...
        # This represents evidence captured before the third-party fallback
        # route was retired. The source transition must retain it, not rewrite
        # or clean it up.
        ...
        historical_claim = ResultClaimInput(
            ...
            score_raw="1200",
            evidence_location={"type": "json_path_v1", "record_path": "$.models[0]"},
            capture_method="legacy_third_party_fallback",
        ...
        ) == (snapshot.id, "model-a", "1200")
        assert decision is not None
        assert (decision.outcome, decision.reason_code) == ("quarantined", "registry_definition_changed")


def test_retired_lmsys_arena_source_never_dispatches_or_writes(seeded_db, monkeypatch):
    with get_session() as session:
        before = {
        ...
        monkeypatch.setattr(
            ingestion_runner,
            "get_adapter",
            lambda *_args, **_kwargs: pytest.fail("inactive LMSYS Arena source must not dispatch an adapter"),
        )

        with pytest.raises(IngestionBlockedError, match="No production-eligible source"):
            run_ingestion(session, source_id="lmsys_arena_leaderboard", dry_run=True)
