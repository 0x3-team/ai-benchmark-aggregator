...
export function CategoryLeaders({
  ...
  const visibleLeaders = categoryFilter
    ? leaders.filter((leader) => leader.category === categoryFilter)
    : leaders;

  if (visibleLeaders.length === 0) return null;
    ...
    <div className="glass mb-3 rounded-xl px-4 py-3">
      ...
      <div className="flex flex-wrap gap-2">
        {visibleLeaders.map((leader) => {
          const modelId = leader.modelId;
          const model = models.find((m) => m.id === modelId);
          const color = categoryDotColor(leader.category);
          if (!modelId || leader.avg == null) {
            return (
              <div
                key={leader.category}
                role="status"
                className="flex items-center gap-2 rounded-lg border border-dashed border-white/15 px-2.5 py-1.5 text-left text-[11px] text-muted-foreground"
                style={{ background: categoryTint(leader.category, 0.05) }}
              >
                <span
                  className="h-2.5 w-2.5 shrink-0 rounded-full"
                  style={{ background: color, boxShadow: `0 0 8px ${color}` }}
                />
                {CATEGORY_LABELS[leader.category]}: no fully covered model ({leader.total} benchmarks)
              </div>
            );
          }
          ...
          return (
            ...
            >
              ...
              <span className="flex flex-col leading-tight">
                ...
                <span className="font-mono text-[10px] text-muted-foreground/70">
                  {leader.n}/{leader.total} benchmarks
                </span>
              </span>
            </button>
          );
