...
| Timestamp-qualified vendor-reported slice (system/model cards, new adapter) | knowledge/math/coding | the frontier numbers users actually search for when the publisher supplies usable freshness evidence (decision ND3) |
...
| ND3 | Vendor-reported slice at launch | **Yes, only for timestamp-qualified claims.** New `vendor_reported` officialness class (ADR-011): snapshot the vendor's own system/model card (PDF/HTML), extract exact reported values with typed evidence, and show a distinct UI badge. A claim needs a publisher-provided timestamp or timestamped immutable revision before it can count toward ND1 or be labeled current. An untimestamped card has `freshness=unknown`, is labeled historical or unknown, and does not count toward freshness-qualified launch coverage. This is a policy widening, not a trust-boundary breach. |
